    ...
    all_words_tensor = torch.tensor(len(all_words), dtype=torch.float32)
    tags_tensor = torch.tensor(len(tags), dtype=torch.float32)

    # Save the tensors along with the model's state_dict
    state_dict = model.state_dict()
    state_dict["all_words"] = all_words_tensor
    state_dict["tags"] = tags_tensor
    save_file(state_dict, "model_and_data.safetensors")
    # tensors = {}
